...
if [ "$1" = '/opt/couchdb/bin/couchdb' ]; then
	...
	if [ "$COUCHDB_ERLANG_COOKIE" ]; then
		cookieFile='/opt/couchdb/etc/.erlang.cookie'
		if [ -e "$cookieFile" ]; then
			if [ "$(cat "$cookieFile" 2>/dev/null)" != "$COUCHDB_ERLANG_COOKIE" ]; then
				echo >&2
				echo >&2 "Warning: $cookieFile contents do not match COUCHDB_ERLANG_COOKIE environment variable"
				echo >&2
			fi
		else
